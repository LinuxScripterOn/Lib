        ...
        function TabElements.Button(config)
            ...
            Btn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                    TweenService:Create(BtnFrame, tweenInfo, {BackgroundColor3 = Color3.fromRGB(30, 30, 36)}):Play()
                    if gradient then
                        gradient:Destroy()
                    end
        ...
        function TabElements.Dropdown(config)
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Code,
                            Text = tostring(opt),
                            TextColor3 = opt == SelectedValue and Color3.fromRGB(205, 205, 205) or Color3.fromRGB(180, 180, 180),
                            TextSize = 14,
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            for _, btn in pairs(DropFrame:GetChildren()) do
                                if btn:IsA("TextButton") then
                                    btn.TextColor3 = btn.Text == tostring(opt) and Color3.fromRGB(205, 205, 205) or Color3.fromRGB(180, 180, 180)
                                end
                            end
            ...
            local function SetValue(value)
                if table.find(Options, value) then
                    ...
                    for _, btn in pairs(DropFrame:GetChildren()) do
                        if btn:IsA("TextButton") then
                            btn.TextColor3 = btn.Text == tostring(value) and Color3.fromRGB(205, 205, 205) or Color3.fromRGB(180, 180, 180)
                        end
                    end
                    Linux:SaveConfig("Dropdown_" .. config.Name, SelectedValue, shouldSave, configFolder)
