...
function Linux:Notify(config)
    ...
    local Notification = Linux.Instance("Frame", {
        ...
        BorderColor3 = Color3.fromRGB(150, 150, 155),
        BorderSizePixel = 2,
...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Toggle(config)
            ...
            local Glow = Linux.Instance("UIGlow", {
                Parent = ToggleBox,
        ...
        function TabElements.Dropdown(config)
            ...
            local Selected = Linux.Instance("TextLabel", {
                ...
                TextColor3 = Color3.fromRGB(100, 59, 154),
                TextSize = 14,
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Code,
                            Text = tostring(opt),
                            TextColor3 = opt == SelectedValue and Color3.fromRGB(100, 59, 154) or Color3.fromRGB(130, 130, 130),
                            TextSize = 14,
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            Selected.TextColor3 = Color3.fromRGB(100, 59, 154)
                            for _, btn in pairs(DropFrame:GetChildren()) do
                                if btn:IsA("TextButton") then
                                    btn.TextColor3 = btn.Text == tostring(opt) and Color3.fromRGB(100, 59, 154) or Color3.fromRGB(130, 130, 130)
                                end
                            end
            ...
            local function SetValue(value)
                if table.find(Options, value) then
                    ...
                    for _, btn in pairs(DropFrame:GetChildren()) do
                        if btn:IsA("TextButton") then
                            btn.TextColor3 = btn.Text == tostring(value) and Color3.fromRGB(100, 59, 154) or Color3.fromRGB(130, 130, 130)
                        end
                    end
                    Linux:SaveConfig("Dropdown_" .. config.Name, SelectedValue, shouldSave, configFolder)
